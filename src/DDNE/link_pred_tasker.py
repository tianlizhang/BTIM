...
class Link_Pred_Tasker():
	...
	def build_prepare_node_feats(self,args,dataset):
		if args.use_2_hot_node_feats or args.use_1_hot_node_feats:
			def prepare_node_feats(node_feats):
				return utils.sparse_prepare_tensor(node_feats,
	...
	def get_sample(self,idx,test, **kwargs):
		...
		label_adj = taskers_utils.get_sp_adj(edges = self.data.edges, 
								  time = idx+1,
								  weighted = False, # 此处让所有的 vals=1，BC-OTC也不会预测边数了
								  time_window =  self.args.adj_mat_time_window)
		if test:
		...
		if self.args.smart_neg_sampling: # True
			existing_nodes = torch.cat(existing_nodes)

		
		if 'all_edges' in kwargs.keys() and kwargs['all_edges'] == True: # when link_perd + valid/test, is all_edges
		...
		grn_adj = taskers_utils.get_sp_adj(edges = self.data.edges, 
								   time = idx+1,
								   weighted = True,
								   time_window = self.args.adj_mat_time_window)
		grn_adj = taskers_utils.normalize_adj(adj = grn_adj, num_nodes = self.data.num_nodes)
		return {'idx': idx,
