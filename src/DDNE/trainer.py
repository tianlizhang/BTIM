...
class Trainer():
	...
	def predict(self, sample):
		neigh_tnr = torch.zeros((self.num_nodes, self.num_nodes)).to(sample.hist_adj_list[0].device)
		for t in range(len(sample.hist_adj_list)):
			neigh_tnr += sample.hist_adj_list[t].to_dense()
		...
		loss = self.comp_loss(adj_est, gnd_tnr.to_dense(), neigh_tnr, nodes_embs) + node_loss
		...
		"""predict_batch_size, node_indices = 100000, sample.label_sp['idx']
		gather_predictions=[]
